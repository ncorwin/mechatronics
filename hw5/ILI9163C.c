...
void LCD_drawCharacter(unsigned short x, unsigned short y, char character,unsigned short color){
    int i,j;
    char val;
    char test;
    for (i=0;i<5;i++){
        val = ASCII[character-32][i];
        for (j=0;j<8;j++){
            test=val>>j;
            test = test&0b1;
            if (test == 1){
...
void LCD_drawString(unsigned short x, unsigned short y, char *string,unsigned short color) {
    ...
    int y_start = y;
    while(string[i] != 0){
        LCD_drawCharacter(x,y,string[i],color);
        x = x+6;
        i++;
        if (x > 123){
            x = x_start;
